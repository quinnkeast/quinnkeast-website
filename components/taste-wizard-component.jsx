...
const BubbleButton = styled.button`
  ...
  color: var(--text);
  line-height: 1;
  transition: transform 0.2s ease, box-shadow 0.2s ease, border-color 50ms ease;
...
const Restart = styled.button`
  ...
  padding: 8px 32px;
  border-radius: 32px;
  border: 2px solid rbga(0, 0, 0, 0.05);
  background: rgba(0, 0, 0, 0);
...
class TasteWizardAffinityComponent extends Component {
  ...
  renderRecipe(image, step, title, subtitle) {
    let likeBtnClass = classNames({
      ...
      disabled:
        this.state.selected === "love" || this.state.selected === "notForMe",
    });

    let loveBtnClass = classNames({
      active: this.state.selected === "love",
      disabled:
        this.state.selected === "like" || this.state.selected === "notForMe",
    });
    ...
    let notForMeBtnClass = classNames({
      ...
      disabled:
        this.state.selected === "love" || this.state.selected === "like",
    });
        ...
        <div>
          ...
          <Actions>
            <BubbleButton
              ...
              onClick={() => this.handleResponse("like")}
              ontouchstart
            >
            ...
            <BubbleButton
              className={loveBtnClass}
              onClick={() => this.handleResponse("love")}
              ontouchstart
            >
              <div className="border"></div>
            ...
            <BubbleButton
              className={notForMeBtnClass}
              onClick={() => this.handleResponse("notForMe")}
              ontouchstart
            >
              <div className="border"></div>
    ...
    let content = () => (
      <>
        <Recipe>
          {currentStep === 1 &&
            this.renderRecipe(
              "/assets/projects/taste-wizard-kung-pao.jpg",
              1,
              "Kung Pao Cauliflower",
              "with Edamame Rice & Peanuts"
            )}
          {currentStep === 2 &&
            this.renderRecipe(
              "assets/projects/taste-wizard-seared-steak.jpg",
              2,
              "Seared Steak & Brown Butter Pan Sauce",
              "with Roasted Carrots & Potatoes"
            )}
        </Recipe>
        {currentStep === 3 && (
          <Restart onClick={() => this.reset()}>↖ Restart</Restart>
        )}
      </>
    );
    ...
    return (
      <Container className="container">
        <Wizard>
          ...
          <WizardTitle className={this.state.currentStep === 3 ? "faded" : ""}>
            Finally, how do you feel about these recipes?
          </WizardTitle>
          <WizardSubtitle
            className={this.state.currentStep === 3 ? "faded" : ""}
          >
            Help us get a sense of what kinds of recipes you like!
          </WizardSubtitle>
          <Recipe>
            {currentStep === 1 &&
              this.renderRecipe(
          ...
          {currentStep === 3 && (
            <Restart onClick={() => this.reset()}>↖ Restart demo</Restart>
          )}
          <Skip className={this.state.currentStep === 3 ? "faded" : ""}>
            Or, skip and see your first box
          </Skip>
        </Wizard>
        <p class="extra-small">
          Meal affinity wizard created in React by Quinn Keast for the purpose
          of this case study.
        </p>
      </Container>
    );
